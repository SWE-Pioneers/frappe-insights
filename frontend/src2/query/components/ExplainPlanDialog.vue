...
<template>
	<Dialog v-model:open="showDialog" :title="__('Query Plan')" size="3xl">
		<div v-if="query.explaining" class="flex items-center justify-center py-10 text-gray-500">
			<span>{{
				__('Running {0}…', query.explainResult?.is_analyze ? 'EXPLAIN ANALYZE' : 'EXPLAIN')
			}}</span>
		</div>
