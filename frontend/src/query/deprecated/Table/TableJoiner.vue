<template>
	<div class="flex w-full flex-col overflow-hidden">
		<div class="flex flex-shrink-0 items-center bg-white pb-3 pt-1">
			...
			<div class="text-sm tracking-wide text-gray-700">{{ __('JOIN') }}</div>
		</div>
		<div class="flex flex-1 flex-col space-y-3 overflow-y-scroll">
			<div class="flex flex-col space-y-3">
				<div class="space-y-1 text-sm">
					<div class="text-gray-700">{{ __('Left Table') }}</div>
				...
				<div class="space-y-1 text-sm">
					<div class="text-gray-700">{{ __('Join Type') }}</div>
					<Autocomplete
						v-model="join.type"
				...
				<div class="space-y-1 text-sm">
					<div class="text-gray-700">{{ __('Right Table') }}</div>
				...
				<div class="text-sm">
					<div class="flex items-end space-x-1">
						<div class="flex-1">
							<div class="mb-1 text-gray-700">{{ __('Left Column') }}</div>
							<Autocomplete
						...
						<div class="flex-1">
							<div class="mb-1 text-gray-700">{{ __('Right Column') }}</div>
							<Autocomplete
			...
			<div class="flex justify-end space-x-2">
				<Button :disabled="!editTable.join" @click="clear_join"> {{ __('Clear') }} </Button>
				<Button
					variant="solid"
...
const joinTypeOptions = ref([
	...
	{ label: __('Full Outer Join'), value: 'full' },
])
