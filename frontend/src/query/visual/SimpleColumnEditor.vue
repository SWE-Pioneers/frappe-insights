...
<template>
	<div class="flex flex-col gap-4 p-4">
		<div class="space-y-1">
			<span class="text-sm font-medium text-gray-700">{{ __('Aggregation') }}</span>
			<Autocomplete
				:modelValue="column.aggregation.toLowerCase()"
				:placeholder="__('Aggregation')"
		...
		<div v-if="FIELDTYPES.DATE.includes(column.type)" class="space-y-1">
			<span class="text-sm font-medium text-gray-700">{{ __('Date Format') }}</span>
			<Autocomplete
